...
import { MAX_FILE_SIZE, ALLOWED_FILE_TYPES, SMALL_FILE_SIZE_LIMIT, SMALL_FILE_SIZE_LIMIT_MB } from '@/lib/constants';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Validate file size
    if (file.size > SMALL_FILE_SIZE_LIMIT) {
      return handleError(
        null,
        `حجم الملف (${(file.size / (1024 * 1024)).toFixed(2)} ميجابايت) يتجاوز الحد الأقصى المسموح به (${SMALL_FILE_SIZE_LIMIT_MB} ميجابايت). للملفات الأكبر، يرجى استخدام خدمة رفع خارجية.`,
        400,
        ErrorType.VALIDATION
      );
    }
    ...
    try {
      ...
      const uploadPromise = put(path, file, {
        access: 'public',
      });
